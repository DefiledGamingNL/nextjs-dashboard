...
export default function UserInfoCard() {
  ...
  async function fetchUserProfile() {
    ...
    const { data: profileData, error: profileError } = await supabase
      .from("profiles")
      .select(`full_name, username, avatar_url, position, phone`)
      .eq("id", userId)
      .single();
  ...
  async function handleSave(event: React.FormEvent) {
    ...
    try {
      ...
      // Email en phone naar undefined converteren als ze null zijn
      const updatedEmail = email ?? undefined;

      // Update de authenticated user (email en phone)
      ...
      // Update het profiel in de 'profiles' tabel
      const { error: profileUpdateError } = await supabase
        .from("profiles")
        .update({
          full_name: fullName,
          position,
          avatar_url: avatar, // Alleen updaten als je avatar ook in de UI aanpast
          phone,
        })
        .eq("id", userId);
      ...
      if (profileUpdateError) {
        closeModal();
        toast.error(`Failed to update profile: ${profileUpdateError.message}`);
        return;
      }
